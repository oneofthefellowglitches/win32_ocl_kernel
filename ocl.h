...
// "typedef int fixed;\n"
// "inline fixed fixed_mul(fixed a, fixed b) { long t = (long)a * (long)b; return (fixed)(t >> 16); }\n"
// "inline fixed fixed_add(fixed a, fixed b) { return (fixed)(a + b); }\n"
// "inline fixed fixed_sub(fixed a, fixed b) { return (fixed)(a - b); }\n"
// "inline fixed fixed_div(fixed a, fixed b) { long t = ((long)a << 16); return (fixed)(t / (long)b); }\n"
// "const fixed FIXED_ONE = (1<<16);\n"
// "const fixed FIXED_LN2 = (fixed)(0.69314718f * 65536.0f);\n"
// "fixed fixed_ln_inline(fixed x) {\n"
// "  if (x <= 0) return (fixed)(-30.0f * 65536.0f);\n"
// "  int shift = 0;\n"
// "  while (x < FIXED_ONE) { x <<= 1; --shift; }\n"
// "  while (x >= (FIXED_ONE<<1)) { x >>= 1; ++shift; }\n"
// "  fixed y = x - FIXED_ONE;\n"
// "  fixed y2 = fixed_mul(y,y);\n"
// "  fixed y3 = fixed_mul(y2,y);\n"
// "  fixed y4 = fixed_mul(y3,y);\n"
// "  const fixed c2 = (fixed)(0.5f * 65536.0f);\n"
// "  const fixed c3 = (fixed)((1.0f/3.0f) * 65536.0f);\n"
// "  const fixed c4 = (fixed)(0.25f * 65536.0f);\n"
// "  fixed ln_mant = fixed_sub(fixed_add(fixed_sub(y, fixed_mul(c2, y2)), fixed_mul(c3, y3)), fixed_mul(c4, y4));\n"
// "  fixed shift_ln2 = fixed_mul((fixed)(shift * 65536), FIXED_LN2);\n"
// "  return fixed_add(ln_mant, shift_ln2);\n"
// "}\n"
// "typedef struct { uchar r,g,b; } color24;\n"
// "__kernel void julia_fixed(__global uint* out, int width, int height, int cx_fp, int cy_fp, int x_min_fp, int x_max_fp, int y_min_fp, int y_max_fp, int max_iter, int ln_ln_bailout_fp, __constant color24* pal) {\n"
// "  size_t gid = get_global_id(0);\n"
// "  size_t total = (size_t)width * (size_t)height; if (gid >= total) return;\n"
// "  int px = (int)(gid % (size_t)width);\n"
// "  int py = (int)(gid / (size_t)width);\n"
// "  int denom_x = (width - 1) > 0 ? (width - 1) : 1;\n"
// "  int denom_y = (height - 1) > 0 ? (height - 1) : 1;\n"
// "  fixed dx = fixed_div((fixed)(x_max_fp - x_min_fp), (fixed)(denom_x << 16));\n"
// "  fixed dy = fixed_div((fixed)(y_max_fp - y_min_fp), (fixed)(denom_y << 16));\n"
...
// "  int iter = 0; int bailout_fp = (4<<16);\n"
...
// "      fixed ln_abs = fixed_div(ln_mag2, (fixed)(2<<16)); if (ln_abs <= 0) ln_abs = (fixed)(65536/1000);\n"
// "      fixed ln_ln_abs = fixed_ln_inline(ln_abs);\n"
// "      fixed numer = fixed_sub(ln_ln_abs, (fixed)ln_ln_bailout_fp);\n"
// "      if (numer <= 0) smooth_fp = fixed_add((fixed)(iter<<16), (fixed)(1<<16));\n"
// "      else { fixed nu = fixed_div(numer, FIXED_LN2); fixed tmp = fixed_sub(fixed_add((fixed)(iter<<16),(fixed)(1<<16)), nu); if (tmp<0) tmp=0; smooth_fp = tmp; }\n"
// "    }\n"
// "  }\n"
// "  fixed norm = fixed_div(smooth_fp, (fixed)(max_iter<<16)); if (norm < 0) norm = 0; if (norm > (fixed)65536) norm = (fixed)65536;\n"
// "  fixed pos_fp = fixed_mul(norm, (fixed)(8<<16));\n"
// "  int idx = (int)(pos_fp >> 16); fixed frac = (fixed)(pos_fp & 0xFFFF);\n"
// "  if (idx < 0) { idx = 0; frac = 0; } if (idx >= 8) { idx = 7; frac = (fixed)(65535); }\n"
// "  { /* Block scope for palette coloring */\n"
// "    color24 a = pal[idx]; color24 b = pal[idx+1];\n"
// "    int rr = (int)((int)a.r + ((((int)b.r - (int)a.r) * (int)frac) >> 16));\n"
// "    int gg = (int)((int)a.g + ((((int)b.g - (int)a.g) * (int)frac) >> 16));\n"
// "    int bb = (int)((int)a.b + ((((int)b.b - (int)a.b) * (int)frac) >> 16));\n"
// "    uint outv = (0xFFu<<24) | ((uint)(rr&255)<<16) | ((uint)(gg&255)<<8) | (uint)(bb&255);\n"
// "    out[gid] = outv;\n"
// "  }\n"
// "}\n";
...
// // AMD wymaga wyrównania struktur do 4 bajtów (uchar4 lub padding)
